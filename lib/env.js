...
const isProduction = baseEnv.NODE_ENV === "production";
...
if (isProduction && !baseEnv.AUTH_JWT_SECRET) {
  throw new Error(
    "AUTH_JWT_SECRET is required in production. Set a strong secret (min 32 chars)."
  );
}

if (isProduction && !baseEnv.AUTH_OTP_SALT) {
  throw new Error(
    "AUTH_OTP_SALT is required in production. Set a strong salt (min 8 chars)."
  );
}

const authJwtSecret =
  baseEnv.AUTH_JWT_SECRET || "dev-only-change-this-secret-key-now";
const authOtpSalt = baseEnv.AUTH_OTP_SALT || "dev-only-change-this-otp-salt";

export const env = {
  ...baseEnv,
  DATABASE_URL_RESOLVED: databaseUrl,
  AUTH_JWT_SECRET: authJwtSecret,
  AUTH_OTP_SALT: authOtpSalt,
};
