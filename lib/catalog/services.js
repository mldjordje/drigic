...
export const getCachedServicesCatalog = unstable_cache(
  async () => {
    ...
    const rows = await db
      .select({
        ...
        showInFaceBooking: schema.services.showInFaceBooking,
        showInBodyBooking: schema.services.showInBodyBooking,
    ...
    const packageItemsRows = packageServiceIds.length
      ? await db
          ...
          .orderBy(
            asc(schema.servicePackageItems.sortOrder),
            asc(schema.servicePackageItems.createdAt)
          )
      : [];

    const packageItemsByPackageId = packageItemsRows.reduce((acc, row) => {
      if (!acc[row.packageServiceId]) acc[row.packageServiceId] = [];
    ...
    const grouped = uniqueRows.reduce((acc, row) => {
      ...
      acc[row.categoryId].services.push({
        ...
        showInFaceBooking: Boolean(row.showInFaceBooking),
        showInBodyBooking: Boolean(row.showInBodyBooking),
        priceRsd: Number(row.priceRsd || 0),
        ...
        promotion:
          row.promoPriceRsd !== null && row.promoPriceRsd !== undefined && row.promoActive
            ? {
                ...
                promoPriceRsd: Number(row.promoPriceRsd),
  ...
  ["public-services"],
  { revalidate: REVALIDATE, tags: [PUBLIC_SERVICES_CACHE_TAG] }
);
