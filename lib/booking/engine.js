...
export const CONSULTATION_SELECTION_ID = "consultation";
...
export async function isWithinWorkHours(startAt, durationMin, settings) {
  ...
  const activationRows = await loadMorningShiftActivations({
    startDate: dateKey,
    endDate: dateKey,
  });
  const workingIntervals = getWorkingIntervalsForDate(dateKey, activationRows);
      ...
      items: [
        {
          ...
          brand: null,
          pricingNote: "2000 RSD se obracunava i eventualno odbija naknadno u ordinaciji.",
        },
      ],
...
export async function getAvailabilityByDay({
  ...
  const workingIntervals = getWorkingIntervalsForDate(date, activationRows);
...
export async function getAvailabilityByMonth({
  ...
  for (let day = 1; day <= lastDay; day += 1) {
    ...
    const slots = buildDaySlots({
      ...
      workingIntervals: getWorkingIntervalsForDate(dayStr, activationsByDate[dayStr] || []),
    });
