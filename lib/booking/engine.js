...
import {
  ...
  loadSaturdayAfternoonActivations,
  loadSundayAvailability,
  mapMorningShiftActivationsByDate,
  mapSaturdayAfternoonActivationsByDate,
...
const HYALURONIC_BRAND_OPTIONS = HYALURONIC_BRANDS.reduce((acc, item) => {
  acc[item.key] = { label: item.label, unitPriceRsd: item.unitPriceRsd };
  return acc;
}, {});
...
export async function isWithinWorkHours(startAt, durationMin, settings) {
  ...
  const [morningRows, saturdayRows, sundayRows] = await Promise.all([
    loadMorningShiftActivations({
      startDate: dateKey,
      endDate: dateKey,
    }),
...
function filterExcludedBookingRows(rows, excludeBookingId) {
  if (!excludeBookingId) {
    return rows;
  }
  return rows.filter((row) => row.id !== excludeBookingId);
}
...
export async function resolveQuote(input = [], options = {}) {
  ...
  const packageRootSelections = new Map(
    rootServiceSelections
      .map((selection) => {
        ...
        return [
          ...
          {
            service: rootService,
            quantity: toSafeQuantity(selection.quantity || 1),
          },
        ];
      })
      .filter(Boolean)
  ...
  packageRootSelections.forEach(({ service, quantity }, packageServiceId) => {
    const packageItems = items.filter(
      (item) => item.sourcePackageServiceId === packageServiceId
    );
    if (!packageItems.length) {
      return;
    }

    const { promoBase } = getActivePromoBase(service, now);
    const packageTotal = Number(promoBase || 0) * quantity;
    const regularTotal = packageItems.reduce(
      (sum, item) => sum + Number(item.regularPriceRsd || 0),
      0
    );
    if (regularTotal <= 0) {
      return;
    ...
    packageItems.forEach((item, index) => {
      if (index === packageItems.length - 1) {
        item.finalPriceRsd = Math.max(0, packageTotal - allocated);
        return;
      }

      const share = Math.round((Number(item.regularPriceRsd || 0) / regularTotal) * packageTotal);
      item.finalPriceRsd = Math.max(0, share);
      allocated += item.finalPriceRsd;
    });
  });
  ...
  let rawTotalDurationMin = items.reduce((sum, item) => sum + item.durationMin, 0);
  packageRootSelections.forEach(({ service, quantity }, packageServiceId) => {
    const packageItems = items.filter(
      (item) => item.sourcePackageServiceId === packageServiceId
    );
    ...
    const expandedDurationMin = packageItems.reduce(
    ...
    rawTotalDurationMin =
      rawTotalDurationMin -
      expandedDurationMin +
      Number(service.durationMin || 0) * quantity;
  });
...
export async function getAvailabilityByDay({
  ...
  const slots = buildDaySlots({
    ...
    totalDurationMin,
    settings,
    existingBookings: [...filterExcludedBookingRows(existingBookings, excludeBookingId), ...existingBlocks],
    workingIntervals,
  });
...
export async function getAvailabilityByDayDuration({ date, totalDurationMin, excludeBookingId }) {
...
export async function getAvailabilityByMonthDuration({ month, totalDurationMin, excludeBookingId }) {
  const safeDuration = Math.max(5, Math.min(MAX_BOOKING_DURATION_MIN, Number(totalDurationMin || 15)));
  const [settings, employee] = await Promise.all([
    getClinicSettings(),
    getDefaultEmployee(),
  ...
  const morningByDate = mapMorningShiftActivationsByDate(
    morningRows,
    firstDayKey,
    lastDayKey
  );
  const saturdayByDate = mapSaturdayAfternoonActivationsByDate(
    saturdayRows,
    firstDayKey,
    lastDayKey
  );
  ...
  const bookingsByDate = filterExcludedBookingRows(monthBookings, excludeBookingId).reduce((acc, booking) => {
  ...
  for (let day = 1; day <= lastDay; day += 1) {
    ...
    const slots = buildDaySlots({
      ...
      workingIntervals: getWorkingIntervalsForDate(
        dayStr,
        [...(morningByDate[dayStr] || []), ...(saturdayByDate[dayStr] || [])],
