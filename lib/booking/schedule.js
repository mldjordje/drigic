...
export async function loadSaturdayAfternoonActivations({
  startDate,
  endDate,
  includeInactive = false,
  db: injectedDb,
  ...
  try {
    const filters = [
      lte(schema.saturdayAfternoonActivations.startDate, endDate),
      gte(schema.saturdayAfternoonActivations.endDate, startDate),
    ];

    if (!includeInactive) {
      filters.unshift(eq(schema.saturdayAfternoonActivations.isActive, true));
    }
    ...
    return await db
      ...
      .where(and(...filters))
      .orderBy(
        asc(schema.saturdayAfternoonActivations.startDate),
        asc(schema.saturdayAfternoonActivations.startTime)
      );
  } catch (error) {
    if (getPgCode(error) === "42P01") {
      return [];
...
export function mapSaturdayAfternoonActivationsByDate(rows = [], startDate, endDate) {
  const mapped = {};
