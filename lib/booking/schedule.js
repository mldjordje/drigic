...
export async function loadSundayAvailability({
  ...
  const db = injectedDb || getDb();
  const startBound = typeof startDate === "string" ? parseDateKey(startDate) : startDate;
  const endBound = typeof endDate === "string" ? parseDateKey(endDate) : endDate;
  ...
  try {
    const filters = [
      gte(schema.sundayAvailability.sundayDate, startBound),
      lte(schema.sundayAvailability.sundayDate, endBound),
    ];
