...
export async function loadSundayAvailability({
  ...
  try {
    // Različiti drajveri mogu vratiti "date" kao string ili Date;
    // a poređenje date kolone sa string/Date može biti nepouzdano kroz cast-ove.
    // Zato učitavamo redove i filtriramo po dateKey u JS.
    const baseFilters = [];
    if (!includeInactive) {
      baseFilters.push(eq(schema.sundayAvailability.isActive, true));
    }
    ...
    return rows.filter((row) => {
      const key =
        typeof row.sundayDate === "string"
          ? String(row.sundayDate).slice(0, 10)
          : row.sundayDate instanceof Date
            ? toBelgradeDateKey(row.sundayDate)
            : String(row.sundayDate || "");
      return /^\d{4}-\d{2}-\d{2}$/.test(key) && key >= startKey && key <= endKey;
    });
  } catch (error) {
    if (getPgCode(error) === "42P01") {
      return [];
...
export function mapSundayAvailabilityByDate(rows = [], startDate, endDate) {
  ...
  for (const row of rows) {
    // pg "date" može doći kao string ili Date; Date → uvek računaj kao Belgrade date-key
    const key =
      typeof row.sundayDate === "string"
        ? String(row.sundayDate).slice(0, 10)
        : row.sundayDate instanceof Date
