...
export const PRIMARY_SITE_URL = "https://drigic.rs";
export const PRIMARY_HOSTNAME = "drigic.rs";

const DEFAULT_SITE_URL = PRIMARY_SITE_URL;
const DEFAULT_DEV_SITE_URL = "http://localhost:3000";

export const SITE_NAME = "Dr Igić Clinic";
export const SITE_DESCRIPTION =
  "Dr Nikola Igić — ordinacija estetske i anti-age medicine u Nišu. Hijaluronski fileri, tretman mimičnih bora, PRP, mezoterapija, skinbusteri. Zakažite online konsultaciju.";
...
function normalizePrimaryHost(value) {
  try {
    const url = new URL(value);
    if (url.hostname === `www.${PRIMARY_HOSTNAME}`) {
...
export function getConfiguredSiteUrl() {
  ...
  for (const candidate of candidates) {
    ...
    return normalizePrimaryHost(normalized);
  }
