...
function escapeLikePattern(value) {
  // Escape only % (wildcard) — _ is kept because SQL LIKE treats it as "any single char"
  // which still correctly matches our markers (e.g. reminder_24h matches reminder_24h).
  // Stripping _ was a bug: "booking:123:reminder_24h" became "booking:123:reminder24h"
  // and the LIKE check never matched, breaking deduplication for all reminder types.
...
export async function deliverBookingAlertToAdmins({
  ...
  const adminEmailSet = new Set(
    admins.map((a) => String(a.email || "").toLowerCase().trim()).filter(Boolean)
  );
  ...
  // Ako inbox email nije već pokriven kroz admin listu, pošalji direktno
  const normalizedInbox = String(inboxEmail || "").toLowerCase().trim();
  if (normalizedInbox && !adminEmailSet.has(normalizedInbox) && emailPayload) {
    try {
      await sendTransactionalEmail({ to: normalizedInbox, ...emailPayload });
    } catch {
      // ne blokirati glavni flow
    }
  }
