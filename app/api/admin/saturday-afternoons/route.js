...
export async function GET(request) {
  ...
  try {
    const data = await db
      .select()
      .from(schema.saturdayAfternoonActivations)
      .orderBy(
        desc(schema.saturdayAfternoonActivations.isActive),
        desc(schema.saturdayAfternoonActivations.startDate),
        desc(schema.saturdayAfternoonActivations.createdAt)
      );
    ...
    return fail(500, error?.message || "Neuspešno učitavanje popodnevnih termina.");
  }
}
...
export async function POST(request) {
  ...
  try {
    const [createdRecord] = await db
      .insert(schema.saturdayAfternoonActivations)
      .values({
    ...
    return fail(500, error?.message || "Neuspešno kreiranje popodnevnih termina.");
  }
}
...
export async function PATCH(request) {
  ...
  try {
    const [existing] = await db
      .select()
      .from(schema.saturdayAfternoonActivations)
      .where(eq(schema.saturdayAfternoonActivations.id, parsed.data.id))
      .limit(1);
    ...
    const [updatedRecord] = await db
      .update(schema.saturdayAfternoonActivations)
      .set({
    ...
    return fail(500, error?.message || "Neuspešno ažuriranje popodnevnih termina.");
  }
}
