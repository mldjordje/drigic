 ...
 * and only balances out over a month. At 10 EUR/day that means up to 20 EUR in one day,
 * so a 50 EUR / 5 day test can burn out in 3 days. This script enforces the cap itself.
 *
 * WHAT IT DOES (run hourly)
 *   1. Sums campaign cost since CAMPAIGN_START_DATE.
 *   2. Pauses the campaign the moment total cost >= TOTAL_BUDGET_EUR.
...
function main() {
  ...
  var capped = totalCost >= TOTAL_BUDGET_EUR;
  if (capped) {
  ...
  var summary =
    'Potroseno: ' + money(totalCost) + ' / ' + money(TOTAL_BUDGET_EUR) +
    ' (' + Math.round((totalCost / TOTAL_BUDGET_EUR) * 100) + '%)\n\n' + lines.join('\n');

  Logger.log(summary);

  if (EMAIL_TO && (capped || totalCost >= TOTAL_BUDGET_EUR * WARN_AT_RATIO)) {
    var subject = capped
      ? 'Google Ads: budzet potrosen, kampanja pauzirana'
      : 'Google Ads: potroseno ' + Math.round((totalCost / TOTAL_BUDGET_EUR) * 100) + '% budzeta';
    if (DRY_RUN) {
...
function pauseWastefulKeywords(campaign, from, to, lines) {
  var it = campaign.keywords()
    ...
    .withCondition('Cost > ' + KEYWORD_WASTE_EUR)
